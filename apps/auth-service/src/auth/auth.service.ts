...
export class AuthService {
 ...
 async findOrCreateGoogleUser(data: {
    ...
    let user = await this.userRepo.findOne({ where: { googleId: data.googleId } });
    ...
    if (!user) {
      ...
      if (user) {
        user.googleId = data.googleId; // link accounts
      } else {
        user = this.userRepo.create({ ...data, isActive: true });
      }
      await this.userRepo.save(user);
    }
